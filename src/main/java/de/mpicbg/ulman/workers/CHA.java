...
public class CHA
{
	...
	throws IOException, ImgIOException
	{
		...
		//go over all FG objects and calc their CHAs
		long noFGs = 0;
		...
		for (int time=0; time < avgFG.size(); ++time)
		{
			...
			{
				cha += (fg - avgBG.get(time)) / stdBG.get(time);
				++noFGs;
			}
		}
		...
		if (noFGs > 0)
		{
			...
			log.info("CHA: Couldn't calculate average CHA because there are no cells labelled.");
